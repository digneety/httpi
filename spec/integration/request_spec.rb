...
describe HTTPI do
  ...
  before :all do
    ...
    HTTPI::Adapter::Rack.mount('localhost', IntegrationServer)
  ...
  after(:all) do
    HTTPI::Adapter::Rack.unmount('localhost')
  end

  shared_examples_for "an HTTP client" do
    it "and send HTTP headers" do
      request = HTTPI::Request.new :url => "http://localhost/x-header"
    ...
    it "and execute an HTTP GET request" do
      response = HTTPI.get "http://localhost", adapter
    ...
    it "and execute an HTTP POST request" do
      response = HTTPI.post "http://localhost", "<some>xml</some>", adapter
      response.body.should include(@example_web_page)
      response.headers["Content-Type"].should include(@content_type)
    end

    it "and execute an HTTP HEAD request" do
      response = HTTPI.head "http://localhost", adapter
      response.code.should == 200
      response.headers["Content-Type"].should include(@content_type)
    end

    it "and execute an HTTP PUT request" do
      response = HTTPI.put "http://localhost", "<some>xml</some>", adapter
      response.body.should include("PUT is not allowed")
      response.headers["Content-Type"].should include(@content_type)
    end

    it "and execute an HTTP DELETE request" do
      response = HTTPI.delete "http://localhost", adapter
  ...
  shared_examples_for "it works with HTTP basic auth" do
    it "and access a secured page" do
      request = HTTPI::Request.new :url => "http://localhost/auth/basic"
  ...
  shared_examples_for "it works with HTTP digest auth" do
    it "and access a secured page" do
      request = HTTPI::Request.new :url => "http://localhost/auth/digest"
